...
-- Original: /Users/alviniomolina/Documents/GitHub/just-daily-ops-platform/scripts/sql/setup-eitje-tables-clean.sql

-- CLEAN EITJE DATABASE TABLES SETUP
...
CREATE TABLE eitje_teams (
    id SERIAL PRIMARY KEY,
    eitje_id INTEGER NOT NULL UNIQUE,
    environment_id INTEGER, -- No foreign key constraint
    name VARCHAR(255) NOT NULL,
    description TEXT,
...
-- ========================================
-- STEP 3: CREATE LABOR/HOURS DATA TABLES
-- ========================================

-- 5. TIME REGISTRATION SHIFTS RAW (Actual worked shifts)
...
-- ========================================
-- STEP 4: CREATE REVENUE DATA TABLES
-- ========================================
...
-- Master data indexes
CREATE INDEX idx_eitje_environments_eitje_id ON eitje_environments(eitje_id);
CREATE INDEX idx_eitje_environments_active ON eitje_environments(is_active);
CREATE INDEX idx_eitje_teams_eitje_id ON eitje_teams(eitje_id);
CREATE INDEX idx_eitje_teams_environment ON eitje_teams(environment_id);
CREATE INDEX idx_eitje_teams_active ON eitje_teams(is_active);
CREATE INDEX idx_eitje_users_eitje_id ON eitje_users(eitje_id);
CREATE INDEX idx_eitje_users_active ON eitje_users(is_active);
CREATE INDEX idx_eitje_shift_types_eitje_id ON eitje_shift_types(eitje_id);
CREATE INDEX idx_eitje_shift_types_active ON eitje_shift_types(is_active);

-- Labor data indexes
CREATE INDEX idx_time_registration_eitje_id ON eitje_time_registration_shifts_raw(eitje_id);
CREATE INDEX idx_time_registration_date ON eitje_time_registration_shifts_raw(date);
CREATE INDEX idx_time_registration_user ON eitje_time_registration_shifts_raw(user_id);
CREATE INDEX idx_time_registration_team ON eitje_time_registration_shifts_raw(team_id);
CREATE INDEX idx_time_registration_environment ON eitje_time_registration_shifts_raw(environment_id);
CREATE INDEX idx_time_registration_date_env ON eitje_time_registration_shifts_raw(date, environment_id);

CREATE INDEX idx_planning_eitje_id ON eitje_planning_shifts_raw(eitje_id);
CREATE INDEX idx_planning_date ON eitje_planning_shifts_raw(date);
CREATE INDEX idx_planning_user ON eitje_planning_shifts_raw(user_id);
CREATE INDEX idx_planning_team ON eitje_planning_shifts_raw(team_id);
CREATE INDEX idx_planning_environment ON eitje_planning_shifts_raw(environment_id);
CREATE INDEX idx_planning_date_env ON eitje_planning_shifts_raw(date, environment_id);

CREATE INDEX idx_hours_aggregated_date ON eitje_hours_aggregated(date);
CREATE INDEX idx_hours_aggregated_environment ON eitje_hours_aggregated(environment_id);
CREATE INDEX idx_hours_aggregated_team ON eitje_hours_aggregated(team_id);
CREATE INDEX idx_hours_aggregated_date_env ON eitje_hours_aggregated(date, environment_id);

-- Revenue data indexes
CREATE INDEX idx_revenue_days_eitje_id ON eitje_revenue_days_raw(eitje_id);
CREATE INDEX idx_revenue_days_date ON eitje_revenue_days_raw(date);
CREATE INDEX idx_revenue_days_environment ON eitje_revenue_days_raw(environment_id);
CREATE INDEX idx_revenue_days_date_env ON eitje_revenue_days_raw(date, environment_id);

CREATE INDEX idx_revenue_aggregated_date ON eitje_revenue_aggregated(date);
CREATE INDEX idx_revenue_aggregated_environment ON eitje_revenue_aggregated(environment_id);
CREATE INDEX idx_revenue_aggregated_date_env ON eitje_revenue_aggregated(date, environment_id);
