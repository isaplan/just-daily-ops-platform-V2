/**
 * Finance Eitje Data Hours ViewModel Layer
 * Business logic and state management for Eitje processed hours data
 */
...
import {
  ...
  fetchEnvironmentIds,
  fetchProcessedShiftsData,
} from "@/lib/services/finance/eitje-data-hours.service";
import type {
  Location,
  LocationOption,
...
export function useEitjeDataHoursViewModel() {
  // UI State
  ...
  const { data: locations = [], isLoading: isLoadingLocations } = useQuery<Location[]>({
    queryKey: ["eitje-hours-locations"],
  ...
  const { data: environmentIds, isLoading: isLoadingEnvIds } = useQuery<number[] | null>({
    queryKey: ["eitje-hours-environments", selectedLocation],
  ...
  const { data, isLoading, error } = useQuery<ProcessedShiftsDataResponse>({
