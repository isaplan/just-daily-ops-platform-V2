/**
 * Finance Eitje Data Planning Shifts ViewModel Layer
 * Business logic and state management for Eitje processed planning shifts data
 */
...
import {
  ...
  fetchEnvironmentIds,
  fetchPlanningShiftsData,
} from "@/lib/services/finance/eitje-data-planning-shifts.service";
import type {
  Location,
  LocationOption,
...
export function useEitjeDataPlanningShiftsViewModel() {
  // UI State
  ...
  const { data: locations = [], isLoading: isLoadingLocations } = useQuery<Location[]>({
    queryKey: ["eitje-planning-shifts-locations"],
  ...
  const { data: environmentIds, isLoading: isLoadingEnvIds } = useQuery<number[] | null>({
    queryKey: ["eitje-planning-shifts-environments", selectedLocation],
    queryFn: () => fetchEnvironmentIds(selectedLocation),
  ...
  const { data, isLoading, error } = useQuery<PlanningShiftsDataResponse>({
    queryKey: ["eitje-planning-shifts-processed", queryFilters, currentPage, environmentIds],
    queryFn: () =>
      fetchPlanningShiftsData({
