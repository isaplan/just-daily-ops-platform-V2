/**
 * Finance Eitje Data Labor Costs ViewModel Layer
 * Business logic and state management for Eitje aggregated labor costs data
 */
...
import {
  ...
  fetchEnvironmentIds,
  fetchLaborCostsData,
} from "@/lib/services/finance/eitje-data-labor-costs.service";
import type {
  Location,
  LocationOption,
...
export function useEitjeDataLaborCostsViewModel() {
  // UI State
  ...
  const { data: locations = [], isLoading: isLoadingLocations } = useQuery<Location[]>({
    queryKey: ["eitje-labor-costs-locations"],
  ...
  const { data: environmentIds, isLoading: isLoadingEnvIds } = useQuery<number[] | null>({
    queryKey: ["eitje-labor-costs-environments", selectedLocation, locations],
  ...
  const { data, isLoading, error } = useQuery<LaborCostsDataResponse>({
    queryKey: ["eitje-labor-costs-aggregated", queryFilters, currentPage, environmentIds],
    queryFn: () =>
      fetchLaborCostsData({
