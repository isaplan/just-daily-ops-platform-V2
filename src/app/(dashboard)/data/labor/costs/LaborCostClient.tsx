...
export function LaborCostClient({ initialData }: LaborCostClientProps) {
  ...
  const filterLabels = useMemo(() => [
    ...
    { key: "worker", label: "Worker", value: viewModel.selectedWorker !== "all" ? viewModel.selectedWorker : null },
    { key: "timePeriod", label: "Time Period", value: viewModel.selectedTimePeriod },
  ], [
  ...
  const handleFilterChange = useCallback((key: string, value: any) => {
    switch (key) {
      ...
      case "timePeriod": viewModel.setSelectedTimePeriod(value as "year" | "month" | "week" | "day"); break;
    }
  }, [
  ...
  const handleFilterRemove = useCallback((key: string) => {
    switch (key) {
      ...
      case "timePeriod": viewModel.setSelectedTimePeriod("day"); break;
    }
  }, [
    viewModel.setSelectedYear,
    viewModel.setSelectedMonth,
      ...
      <div className="space-y-6">
        ...
        {!viewModel.isLoading && !viewModel.error && viewModel.laborCostData && viewModel.laborCostData.length > 0 && viewModel.aggregatedCosts && (
          <AggregatedCostsSummary
            ...
            metrics={[
              { label: "Avg Hour", value: viewModel.aggregatedCosts.avgHour, format: "number", decimals: 2 },
              { label: "Avg Cost", value: viewModel.aggregatedCosts.avgCost, format: "currency" },
            ]}
          />
        )}
