...
export function WorkersClient({ initialData }: WorkersClientProps) {
  const viewModel = useWorkersV2ViewModel(initialData);
  ...
  const filterLabels = useMemo(() => [
    ...
    { key: "team", label: "Team", value: teamFilter !== "all" ? teamFilter : null },
    { key: "contractType", label: "Contract Type", value: viewModel.contractTypeFilter !== "all" ? viewModel.contractTypeFilter : null },
    { key: "workerSearch", label: "Worker", value: workerSearch || null },
  ], [
    viewModel.selectedYear,
    viewModel.selectedMonth,
    viewModel.selectedDay,
    viewModel.selectedLocation,
    viewModel.contractTypeFilter,
    teamFilter,
    workerSearch,
  ]);
  ...
  const handleFilterChange = useCallback((key: string, value: any) => {
    switch (key) {
      ...
      case "team": setTeamFilter(value); break;
      case "contractType": viewModel.setContractTypeFilter(value); break;
      case "workerSearch": setWorkerSearch(value); break;
    }
  }, [
    ...
    viewModel.setContractTypeFilter,
  ]);
  ...
  const handleFilterRemove = useCallback((key: string) => {
    switch (key) {
      ...
      case "team": setTeamFilter("all"); break;
      case "contractType": viewModel.setContractTypeFilter("all"); break;
      case "workerSearch": setWorkerSearch(""); break;
    }
  }, [
    viewModel.setSelectedYear,
    viewModel.setSelectedMonth,
          ...
          <>
            <UITable stickyHeader={true} stickyFirstColumn={true}>
              <TableHeader className="!sticky !top-0">
                <TableRow>
                  {/* MERGE COLUMN DISABLED - Commented out per user request */}
                ...
                {viewModel.isCreating && viewModel.editingProfile && (
                  <TableRow className="bg-blue-50">
                    {/* MERGE COLUMN DISABLED - Empty cell commented out per user request */}
                ...
                {filteredWorkers && filteredWorkers.length > 0 ? (
                  filteredWorkers.map((record: WorkerProfile, index: number) => {
                    ...
                    >
                      {/* INLINE EDITING DISABLED - Commented out per user request */}
                      ...
                      {/* Always show view mode - inline editing disabled */}
                      {(
                        // View Mode
                        <>
                          {/* MERGE COLUMN DISABLED - Commented out per user request */}
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              {/* INLINE EDITING DISABLED - Edit button commented out per user request */}
                              {/* <Button
                                size="sm"
                                variant="outline"
                  ...
                  <TableRow>
                    <TableCell 
                      colSpan={viewModel.showAllColumns ? 13 : 10} 
