/**
 * Transaction Analysis - Server Component
...
import { TransactionAnalysisClient } from './TransactionAnalysisClient';
...
export default async function TransactionAnalysisPage() {
  ...
  const [initialTransactionData, locations] = await Promise.all([
    getTransactionAnalysis(startDate, endDate, 1, 50, {}).catch(() => null),
    getLocations().catch(() => []),
  ]);
  ...
  return (
    <TransactionAnalysisClient
      initialData={{
        transactionData: initialTransactionData || undefined,
        locations: locations || [],
      }}
    />
  );
