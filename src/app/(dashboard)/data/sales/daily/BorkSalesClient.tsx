          ...
          {rest.length > 0 && (
            <CollapsibleTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="border rounded-sm bg-white border-black text-black hover:bg-blue-500 hover:border-blue-500 hover:text-white inline-flex items-center gap-1"
              >
...
export function BorkSalesClient({ initialData }: BorkSalesClientProps) {
  ...
  const filterLabels = useMemo(() => [
    ...
    { key: "category", label: "Category", value: viewModel.selectedCategory !== "all" ? viewModel.selectedCategory : null },
  ], [
  ...
  const handleFilterChange = useCallback((key: string, value: any) => {
    switch (key) {
      ...
      case "category": viewModel.setSelectedCategory(value); break;
    }
  }, [
  ...
  const handleFilterRemove = useCallback((key: string) => {
    switch (key) {
      ...
      case "category": viewModel.setSelectedCategory("all"); break;
    }
  }, [
    viewModel.setSelectedYear,
    viewModel.setSelectedMonth,
  ...
  // Use totals from ViewModel (MVVM: business logic in ViewModel, not View)
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      {!viewModel.isLoading && !viewModel.error && viewModel.salesData && viewModel.salesData.records && (
        <>
          ...
          {viewModel.salesTotals && (
            <AggregatedCostsSummary
              title="Summary"
              startDate={viewModel.startDate}
              endDate={viewModel.endDate}
              metrics={[
                { label: "Total Revenue", value: viewModel.salesTotals.totalRevenue, format: "currency" },
                { label: "Total Products", value: viewModel.salesTotals.totalProducts, format: "number", decimals: 0 },
                { label: "Total Quantity", value: viewModel.salesTotals.totalQuantity, format: "number", decimals: 0 },
                { label: "Total Transactions", value: viewModel.salesTotals.totalTransactions, format: "number", decimals: 0 },
                { label: "Avg Transaction Value", value: viewModel.salesTotals.avgTransactionValue, format: "currency" },
              ]}
            />
          )}
