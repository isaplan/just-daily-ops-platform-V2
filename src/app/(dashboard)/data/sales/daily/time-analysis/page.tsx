/**
 * Time-Based Analysis - Server Component
...
import { TimeBasedAnalysisClient } from './TimeBasedAnalysisClient';
...
export default async function TimeBasedAnalysisPage() {
  ...
  const [initialTimeData, locations] = await Promise.all([
    getTimeBasedAnalysis(startDate, endDate, {}).catch(() => null),
    getLocations().catch(() => []),
  ]);

  return (
    <TimeBasedAnalysisClient
      initialData={{
        timeData: initialTimeData || undefined,
        locations: locations || [],
      }}
    />
  );
