...
import { FilterDrawer } from "@/components/navigation/filter-drawer";

interface TopnavFilterDrawerV2Props {
  onActiveFiltersChange?: (filters: Array<{ key: string; label: string; value: string | number | null }>) => void;
}

export function TopnavFilterDrawerV2({ onActiveFiltersChange }: TopnavFilterDrawerV2Props) {
  ...
  return (
    <>
      <Button
        ...
        title="Filter V2 (Drawer)"
      >
        <Filter className="h-4 w-4" />
        <span className="absolute -bottom-1 -right-1 h-4 w-4 bg-primary text-primary-foreground text-[8px] font-bold rounded-full flex items-center justify-center leading-none">
          V2
        </span>
      ...
      <FilterDrawer open={isDrawerOpen} onOpenChange={setIsDrawerOpen} />
    </>
  );
}
