...
export default function ProductsOPPPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Create payment session directly with OPP
      ...
      if (data.checkoutUrl) {
        // Redirect directly to OPP's hosted checkout
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Products (OPP Checkout)</h1>
          <p className="text-muted-foreground mt-1">
            Browse products and checkout directly with Online Payment Platform
          </p>
        </div>
        <div className="flex items-center gap-4">
          {cartItemCount > 0 && (
            <div className="text-right">
              ...
              <div className="text-lg font-bold">EUR {cartTotal.toFixed(2)}</div>
            </div>
          )}
          <Button 
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                Checkout with OPP ({cartItemCount})
              </>
            )}
          </Button>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {cartItemCount} item{cartItemCount !== 1 ? 's' : ''} • EUR {cartTotal.toFixed(2)}
                </p>
              </div>
              <Button
                variant="outline"
